# PRE-SWAP Devnet: Setup up your validator and join *bitcanna-dev-1*
...
4. **Download the Genesis** `genesis.json` file
    ```
    cd $HOME
    curl -s https://raw.githubusercontent.com/BitCannaGlobal/testnet-bcna-cosmos/main/instructions/bitcanna-dev-1/genesis.json > ~/.bcna/config/genesis.json
    ```
...
5. **Add to _config.toml_ file the server SEEDs:**
    ...
    ```
    sed -E -i 's/seeds = \".*\"/seeds = \"de9e9250c0fa928dc6311a744eb03a7d4f354bfd@188.166.126.81:26656\"/' $HOME/.bcna/config/config.toml
    ```
