# Devnet-6: Setup up your validator and join *bitcanna-dev-6*
...
We have created this DevNet to research about the migration from Cosmos SDK v0.45.x to v0.46.x

### We are going to work in three new testnets:
* bitcanna-dev-5 (with `bcnad` v1.5.1-pre version) YOU DON'T NEED TO RUN THIS CHAIN.
...
## Running a validator on **bitcanna-dev-6**
...
By running the statesync script we download the latest binary (v2.0.0-beta) and sync the chain to the latest block.
1. **Download the statesync script** for new peers from Github:
    ```
    cd ~
    wget https://raw.githubusercontent.com/BitCannaGlobal/cosmos-statesync_client/main/statesync_DEVNET-6_client_linux_new.sh
    ```
2. **Run the script**: 
    ```
    bash statesync_DEVNET-6_client_linux_new.sh
    ```
...
To create a validator you need a funded wallet. Once the wallet is created ask an admin for Tesnet-funds 
...
bcnad tx staking create-validator \
	...
	--chain-id bitcanna-dev-6 \
	--gas auto \
	--gas-adjustment 1.5 \
	--gas-prices 0.001ubcna
```

You can check the list of validators (also in [Explorer](https://explorer-devnet-6.bitcanna.io/bitcanna/staking)):
